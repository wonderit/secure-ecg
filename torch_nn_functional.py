...
def pool1d(tensor, kernel_size: int = 2, stride: int = 2, mode="max"):
    assert len(tensor.shape) < 5
    if len(tensor.shape) == 1:
        return _pool1d(tensor, kernel_size, stride, mode)
    if len(tensor.shape) == 2:
        return torch.squeeze(pool1d(torch.unsqueeze(tensor, dim=0), kernel_size, stride, mode))
    batches = tensor.shape[0]
    channels = tensor.shape[1]
    out_shape = (
        batches,
    ...
    for batch in range(batches):
        for channel in range(channels):
            result.append(_pool1d(tensor[batch][channel], kernel_size, stride, mode))
    result = torch.stack(result).reshape(out_shape)
    return result
...
def maxpool1d(tensor, kernel_size: int = 2, stride: int = 2):
    return pool1d(tensor, kernel_size, stride)
